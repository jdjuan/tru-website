...
<nav class="navbar navbar-default navbar-static-top">
		<div class="container">
			...
			<div id="navbar" class="navbar-collapse collapse">
				<ul class="nav navbar-nav">
					...
					<li class="active"><a href="#">Slider (Proyectos)</a></li>
	...
	<div class="container">
		<div class="row">
			<h3>Gestión de Sliders de Proyectos</h3>
		</div>
		<div class="row">
			<p>
			...
			<table class="table table-striped table-bordered">
				...
				<tbody>
					...
					$db = new Db();   
					$rows = $db -> select("SELECT * FROM `slidersproyectos` order by idproyecto, orden");
					foreach ($rows as $row) {
					$proyecto = $db -> select("SELECT tituloCorto FROM `proyectos` WHERE id=".$row["idproyecto"]);
						echo '<tr>';
						echo '<td>'. $row['id'] . '</td>';
						echo "<td><img class='backEndImage' src='uploads/". $row["imagen"] . "'></td>";
						echo '<td>'. $proyecto[0]["tituloCorto"] . '</td>';
