		...
		if (!$validate->isFileEmpty($file2)) {
			if ($validate->validateFile($file)) {
				if ($validate->validateFile($file2)) {
					$db -> query("INSERT INTO `lineas` (`icono`,`slide`,`titulo`,`descripcion`,`orden`) 
						VALUES (" . $db->quote($fileName) . "," . $db->quote($fileName2) . "," . $db->quote($titulo) ."," . $db->quote($descripcion) ."," . $orden .")");
