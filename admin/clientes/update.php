...
if (!empty($_POST)) {
    ...
    if (!$validate->isFileEmpty($file)) {
        if ($validate->validateFile($file)) {
            $db -> query("UPDATE clientes set imagen = " . $db->quote($fileName) . 
                ", orden = " .$orden. 
                ", url = " .$db->quote($url). 
                " WHERE id = " . $id);
            header("Location: index.php");
        ...
        $result = $db -> query("UPDATE clientes set orden = " .$orden. ", url = " .$db->quote($url). " WHERE id = " . $id);
        header("Location: index.php");
    }
}

$rows = $db -> select("SELECT * FROM clientes where id = " . $id);
